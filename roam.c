...
static void config_add_paths(struct config* cfg, const char* paths) {
  ...
  while (tok && cfg->writable_count < MAX_WRITABLE) {
    ...
    if (canon) cfg->writable[cfg->writable_count++] = canon;
    tok = strtok(NULL, " \t");
  }
  free(buf);
}
...
static void config_load(struct config* cfg) {
  ...
  struct stat st;
  if (fstat(fileno(f), &st) == -1) die("fstat(" CONFIG_PATH ")");
  if (st.st_uid != 0) {
  ...
  if (!got_writable) config_add_paths(cfg, DEFAULT_WRITABLE);
}

static void config_free(struct config* cfg) {
  for (int i = 0; i < cfg->writable_count; i++) free(cfg->writable[i]);
  free(cfg->shell);
  free(cfg->user);
}
...
static int cap_in_effective(int cap) {
  ...
  if (syscall(__NR_capget, &hdr, data) == -1) return 0;

  return !!(data[cap / 32].effective & (1U << (cap % 32)));
}

static int cap_set_epi(__u32 eff0, __u32 eff1, __u32 prm0, __u32 prm1, __u32 inh0, __u32 inh1) {
  struct __user_cap_header_struct hdr = {
      .version = _LINUX_CAPABILITY_VERSION_3,
      .pid = 0,
  };
...
int main(int argc, char* argv[]) {
  ...
  if (cap_set_epi(dac_bit | setpcap_bit, 0, /* effective  */
  ...
  if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_DAC_READ_SEARCH, 0, 0) == -1) die("prctl(PR_CAP_AMBIENT_RAISE)");
  ...
  if (cap_in_effective(CAP_SETPCAP)) {
    for (int c = 0; c < 64; c++) {
      if (c == CAP_DAC_READ_SEARCH) continue;
      if (prctl(PR_CAPBSET_DROP, c, 0, 0, 0) == -1) {
        if (errno == EINVAL) break; /* past last known capability */
      }
    }
  }

  /* 2d. Drop CAP_SETPCAP — only keep DAC_READ_SEARCH going forward. */
  if (cap_set_epi(dac_bit, 0, /* effective  */
                  dac_bit, 0, /* permitted  */
                  dac_bit, 0  /* inheritable */
                  ) == -1)
  ...
  if (abi == -1) {
    if (errno == ENOSYS || errno == EOPNOTSUPP)
      fprintf(stderr,
              "roam: Landlock not supported by this kernel "
              "(need 5.13+)\n");
    else
  ...
  static const __u64 file_compat = LANDLOCK_ACCESS_FS_READ_FILE | LANDLOCK_ACCESS_FS_WRITE_FILE |
  ...
  if (ruleset_fd == -1) die("landlock_create_ruleset");
  ...
  for (int i = 0; i < cfg.writable_count; i++) {
    ...
    pb.parent_fd = fd;
    pb.allowed_access = S_ISDIR(path_st.st_mode) ? rw_dir_access : rw_file_access;
    err = ll_add_rule(ruleset_fd, LANDLOCK_RULE_PATH_BENEATH, &pb, 0);
    close(fd);
    if (err)
      fprintf(stderr,
              "roam: note: landlock rule '%s': %s "
              "(skipped)\n",
              cfg.writable[i], strerror(errno));
  }
  ...
  if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0)) die("prctl(PR_SET_NO_NEW_PRIVS)");

  /* Enforce the Landlock ruleset on this process and all descendants. */
  if (ll_restrict_self(ruleset_fd, 0)) die("landlock_restrict_self");
  close(ruleset_fd);
  ...
  fprintf(stderr,
          ...
          "  Writable exceptions:",
          cfg.user, abi);
  for (int i = 0; i < cfg.writable_count; i++) fprintf(stderr, " %s", cfg.writable[i]);
  fprintf(stderr, "\n  Type 'exit' to return.\n");
